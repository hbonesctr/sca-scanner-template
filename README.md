# DoD Security Scanner

**Version:** 2.3  
**Author:** Hector L. Bones  
**Last Updated:** 2026-05-09

A browser-based, mobile-friendly security scanning pipeline for validating open source and customer-provided software against Department of Defense compliance requirements. Runs entirely inside GitHub Actions — no local tools, no CLI, no elevated privileges required.
...
1. `.github/workflows/scan.yml` — paste the contents of `scan.yml`
1. `.github/workflows/cleanup.yml` — paste the contents of `cleanup.yml`
1. `cleanup-triggers/cleanup.trigger` — paste the contents of `cleanup.trigger`
...
1. Tap **DoD Security Scanner** in the left list
1. Tap the latest run to see progress
1. All five scan jobs run in parallel — typical total time is 3–6 minutes
...
1. Find the latest `DoD Security Scan Complete - Run #N` release
...
Snyk and Grype can detect vulnerabilities in the following when manifests are present:
...
Semgrep scans source code regardless of language — no manifest required.  
...
|-----------------------|-------------|------------------------|
|GitHub Actions minutes |2,000 min    |~4–6 min                |
...
- Workflow runs with minimum required permissions (`contents: write`, `actions: read`)
